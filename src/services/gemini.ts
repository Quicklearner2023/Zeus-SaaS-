...
export async function sendMessageToAgentStream(
  ...
  const contents: Content[] = [
    ...
    const config = {
    tools: tools,
    systemInstruction: `You are the Lead Orchestrator Agent for a personal SaaS development suite. Your mission is to assist in the design, development, and deployment of web applications, specifically targeting deployment on Netlify.

OPERATIONAL GUIDELINES:
1. ORCHESTRATION: Use the 'start_ai_agent' tool to deploy specialized technical sub-agents (e.g. 'Frontend Architect', 'Backend API Specialist', 'DevOps/Netlify Expert', 'Database Specialist').
2. MASTER PROMPT STRATEGY: When initiating any new feature or project, act as the lead architect, breaking requirements into modular technical tasks for your sub-agents.
3. DEVELOPMENT STANDARDS: All generated code must be production-ready, scalable, follow DRY principles, and include clear documentation.
4. DEPLOYMENT FOCUS: Maintain an expert-level understanding of Netlify, including netlify.toml configurations, edge functions, and continuous deployment workflows.
5. VERSATILITY: While your primary focus is Web Development & Agent Orchestration, retain your ability to analyze business data and process operations. If asked about user metrics, sales, or analytics, leverage tools like 'analyze_sales_performance', 'generate_yearly_report', or 'create_operations_dashboard'.
6. COMMUNICATION: Be concise. Act as a high-level project manager who translates human requirements into technical instructions for your sub-agents.
7. TOOL USAGE: Prioritize tool use to accomplish tasks. Use 'start_ai_agent' to delegate work to specialized sub-agents. Use 'generate_yearly_report' when requested to compile reports, and 'create_operations_dashboard' when requested to build visual dashboards.
`,
  };
  ...
  try {
    ...
    // Evaluate Database Requirements and Project DB Decision
    const dbDecision = evaluateDatabaseRequirements('Requested Software Project', newMessage);
    PLATFORM_STATE.dbDecisions.push(dbDecision);

    // High risk action check
    const riskAnalysis = isHighRiskAction(newMessage);
    ...
    if (riskAnalysis.isHighRisk) {
      approvalReq = {
        ...
        projectId: dbDecision.projectId,
        actionType: riskAnalysis.riskType as any,
        title: `Approval Required: ${riskAnalysis.riskType?.toUpperCase().replace('_', ' ')}`,
        reason: riskAnalysis.reason || 'High-impact operation requested.',
        estimatedImpact: 'Permanent modification to database schema, infrastructure, or git repository.',
        status: 'pending',
        createdAt: new Date().toISOString()
      };
      PLATFORM_STATE.approvalRequests.push(approvalReq);
    }

    // Sanitize any sensitive key leaks in final text
    const sanitizedText = sanitizeText(finalFullText || "");

    const modelMsg: ChatMessage = {
      role: "model",
      parts: lastAggregatedParts.length > 0 ? lastAggregatedParts : [{ text: sanitizedText }],
      timestamp: new Date(),
