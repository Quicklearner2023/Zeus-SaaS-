...
export const ProjectsView: React.FC = () => {
  ...
  return (
    <div className="flex-1 h-screen overflow-y-auto bg-[#FAFAFA] p-6 md:p-8 space-y-8">
      ...
      {/* Projects Grid */}
      {isLoading ? (
        <div className="flex flex-col items-center justify-center py-20 bg-white rounded-3xl border border-black/[0.04] space-y-4">
          <div className="w-8 h-8 border-2 border-zinc-200 border-t-black rounded-full animate-spin" />
          <p className="text-xs font-bold text-zinc-400 uppercase tracking-widest">Querying Platform Database...</p>
        </div>
      ) : projects.length === 0 ? (
        <div className="flex flex-col items-center justify-center py-20 bg-white rounded-3xl border border-black/[0.04] space-y-3">
          <div className="p-4 bg-zinc-50 rounded-2xl">
            <Layers size={32} className="text-zinc-300" />
          </div>
          <div className="text-center">
            <h3 className="font-bold text-zinc-900">No active projects found</h3>
            <p className="text-xs text-zinc-500 mt-1 max-w-[240px] mx-auto">
              Your platform database is currently empty. Use the Chat Orchestrator or the "New Project" button to initialize your first project.
            </p>
          </div>
        </div>
      ) : (
